...
サブスクリプションは、ダッシュボードに関する定期通知を指定したチャンネルに送信する機能です。
定期レポートのリマインドや運用通知に活用できます。

## 画面構成

![サブスクリプション一覧画面](../assets/screenshots/subscriptions.png)
...
| 形式 | 説明 | 用途 |
|------|------|------|
| **PDF** | 通知本文に表示される形式（現状は添付なし） | 印刷、アーカイブ（手動エクスポート推奨） |
| **PNG** | 通知本文に表示される形式（現状は添付なし） | チャットツール、プレゼン資料（手動エクスポート推奨） |
...
現状のサブスクリプションは、ダッシュボードの PDF/PNG を生成して添付するのではなく、通知本文として以下を送信します。

- ダッシュボード名
- スケジュール
- 形式（PDF/PNG）

> **補足**: 実際の PDF/PNG が必要な場合は、ダッシュボード画面の **Export** を利用してください。
...
2. **通知の到達性**: Webhook/SMTP 設定が正しいこと、必要に応じて通知チャンネルのテスト送信で疎通確認することを推奨します
